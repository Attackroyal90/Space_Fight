...
public class ChunkGeneratorMoon implements IChunkGenerator
{
	...
	protected static final IBlockState MAIN_BLOCK = BlockInit.MOON_ROCK.getDefaultState();
    ...
    private static final double MOON_GRAVITY = 0.16; 
    ...
    public ChunkGeneratorMoon(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        this.world = world;
        ...
        ChunkGeneratorMoon.islandNoise = new NoiseGeneratorSimplex(this.rand);
        ...
        ChunkGeneratorMoon.islandNoise = ctx.getIsland();
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            abyte[i] = (byte)Biome.getIdForBiome(BiomeInit.MOON_GROUND);
        }
    ...
    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
    {
        ...
        for (int i = -12; i <= 12; ++i)
        {
            for (int j = -12; j <= 12; ++j)
            {
                ...
                if (k * k + l * l > 4096L && ChunkGeneratorMoon.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
                {
    ...
    {
        return (long)x * (long)x + (long)z * (long)z > 4096L && ChunkGeneratorMoon.getIslandHeightValue(x, z, 1, 1) >= 0.0F;
    }
    ...
    private double[] getHeights(double[] noiseField, int x, int y, int z, int xSize, int ySize, int zSize)
    {
        ...
        for (int l = 0; l < xSize; ++l)
        {
            ...
            {
                float f = ChunkGeneratorMoon.getIslandHeightValue(i, j, l, i1);
    ...
    public void populate(int x, int z)
    {
        ...
        if (i > 4096L)
        {
            float f = ChunkGeneratorMoon.getIslandHeightValue(x, z, 1, 1);

            if (ChunkGeneratorMoon.getIslandHeightValue(x, z, 1, 1) > 40.0F)
            {
